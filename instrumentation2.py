...
#import inspect
#
#def filter_kwargs(func, input_dict):
...
#    #if has_var_keyword:
#    #    return input_dict.copy()  # All keys are allowed if**kwargs exists
# 
#    # Allow only POSITIONAL_OR_KEYWORD and KEYWORD_ONLY parameters
#    allowed_kinds = {inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.KEYWORD_ONLY}
#    allowed_params = [param.name for param in params if param.kind in allowed_kinds]
# 
#    # Filter input_dict to include only allowed parameters
#    return {k: v for k, v in input_dict.items() if k in allowed_params}
...
def ASI(ax, glat=None, glon=None, elev=None, alt=None, **plotting_params):
    ...
    #fov_lat, fov_lon = generate_asi_fov(params['glat'], params['glon'], params['elev'], params['alt'])
    fov_lat, fov_lon = generate_asi_fov(glat, glon, elev, alt)
    #ax.plot(fov_lon, fov_lat, color=params['color'], label=params['name'], linewidth=3, zorder=6.5, transform=ccrs.Geodetic())

    #print(plotting_params.keys())

    nonplotkw = ['name','sites']
    for kw in nonplotkw:
        del plotting_params[kw]
    #plotting_params = params.copy()
    #del plotting_params['glat']
    #del plotting_params['glon']
    #del plotting_params['elev']
    #del plotting_params['alt']
    #del plotting_params['name']
    #del plotting_params['sites']
    #plotting_params2 = filter_kwargs(ax.plot, plotting_params)
    
    #print(plotting_params2)
